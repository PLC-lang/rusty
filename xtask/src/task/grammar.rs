...
mod ast_src;
mod codegen;
use crate::{project_root, task::grammar::{ast_src::{AstEnumSrc, AstNodeSrc, AstSrc, Cardinality, Field, KindsSrc, generate_kind_src}, codegen::*}};

// use self::ast_src::{AstEnumSrc, AstNodeSrc, AstSrc, Cardinality, Field, KindsSrc};

pub(crate) fn generate() {
    let check = false; //TODO reactivate
    let grammar = fs::read_to_string(project_root().join("compiler/plc_rowan_syntax/iec61131_3.ungram"))
        .unwrap()
        .parse()
        .unwrap();
    ...
    let syntax_kinds_file = project_root().join("compiler/plc_rowan_parser/src/syntax_kind/generated.rs");
    ...
    let ast_tokens_file = project_root().join("compiler/plc_rowan_syntax/src/ast/generated/tokens.rs");
    ensure_file_contents(
    ...
    let ast_nodes_file = project_root().join("compiler/plc_rowan_syntax/src/ast/generated/nodes.rs");
    ensure_file_contents(
...
fn generate_syntax_kinds(grammar: KindsSrc) -> String {
    ...
    let result = add_preamble(reformat(ast.to_string()));
...
fn to_upper_snake_case(s: &str) -> String {
    // If already in UPPER_SNAKE_CASE, leave it as-is
...
fn lower_rule(acc: &mut Vec<Field>, grammar: &Grammar, label: Option<&String>, rule: &Rule) {
    ...
    match rule {
        ...
        Rule::Token(token) => {
            assert!(label.is_none(), "tokens can't be labeled: {label:?}");
        ...
        Rule::Rep(inner) => {
            ...
            panic!("unhandled rule: {rule:?} {label:?}");
        }
