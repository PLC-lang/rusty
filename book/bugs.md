...
P0 compiler/plc_header_generator/src/header_generator/type_helper/type_helper_c.rs:58:17: The generated C header declares every date and time type as `time_t`
`TIME`, `DATE`, `TIME_OF_DAY`, and `DATE_AND_TIME` are 32 bit in the compiler (a struct of `TIME`, `LTIME`, `DATE`, `TOD` generates `%Times = type { i32, i64, i32, i32 }`), but the header generator maps every type whose nature is `Date` or `Duration` to `time_t`, which is 64 bit on a current Linux target. A C implementation that includes the generated header builds a struct with a different layout than the Structured Text side: every member after the first short time member sits at another offset, and both sides read wrong values. The same applies to a `VAR_INPUT` of type `TIME`, which the header declares as a 64-bit parameter. Expected: a fixed-width type per family, for example `uint32_t` for the short types and `int64_t` for the long ones.
```
// Reproducible example:
...
P0 libs/stdlib/src/num_conversion.rs:6:13: Real to integer conversion rounds the exact half away from zero instead of to the even number
`REAL_TO_DINT(2.5)` returns `3`, `REAL_TO_DINT(3.5)` returns `4`, and `REAL_TO_DINT(-2.5)` returns `-3`. The `define_rounding_conversion` macro implements every `*_TO_*INT` conversion of the standard library as `input.round() as $output_ty`, and `f32::round` and `f64::round` round the exact half away from zero. IEC 61131-3 defines the exact half case for these conversions as rounding to the nearest even number, so the expected results are `2`, `4`, and `-2`. Every other input rounds the same in both rules, so the difference is silent and appears only at `.5`. Expected: `round_ties_even`, which both float types provide.
```
// Reproducible example:
...
FUNCTION main : DINT
    ...
    refA REF= instA;
    refA.refB REF= instB;
    refA.refB(x := 7); // observed: E048 "Could not resolve reference to x"; with refA.refB(7) the compiler panics
                       // expected: prints "B body x=7", like refA^.refB^(x := 7) does with REF_TO
END_FUNCTION
```
...
P2 compiler/plc_header_generator/src/header_generator/file_helper.rs:84:9: --header-prefix writes every unit to the same header, so only the last one survives
A build description with `api.st` and `b.st` gives `api.h` and `b.h` from `plc generate plc.json headers`, but `plc generate plc.json headers --header-prefix pre` writes one `pre.h` that declares only the function of `b.st`; the `scale` function of `api.st` is lost and nothing is reported. The prefix branch returns `format!("{}.{}", prefix, file_extension)` as the output name of every unit, so each unit overwrites the file that the unit before it wrote. Expected: the prefix is combined with the source name, or the units are combined into one header as `--generate-headers -o <name>` does.
```
// Reproducible example:
// plc generate plc.json headers                     -> api.h and b.h
// plc generate plc.json headers --header-prefix pre -> pre.h only
...
P2 src/validation/statement.rs:416:9: ADR or REF of an expression passes --check and stops codegen without naming a rule
`l := ADR(i + 1);` and `l := ADR(5);` produce no diagnostic with `plc --check`, and `plc --ir` stops with `error: Builder error: ()` at the call, which names neither a rule nor a fix. E066 "Invalid address-of operation" covers exactly this case, but its only producer, `validate_address_of_expression`, is called from the `ReferenceAccess::Address` arm alone, and no source spelling produces that access kind. The validation function that the `ADR` built-in declares only counts the arguments and checks them for constants. `REF(i + 1)` behaves the same. Expected: E066 on the argument, raised from the built-in's own validation function.
```
// Reproducible example:
// plc --check: no diagnostic, exit 0
...
P2 compiler/plc_lowering/src/initializer.rs:1022:21: An FB_INIT method with parameters is accepted and called with an undefined argument
`FB_INIT` is the constructor hook of a function block and is called by the generated `<Type>__ctor` without arguments (`create_call_statement` with `None` parameters). A declaration that gives `FB_INIT` a `VAR_INPUT` passes `--check` without any diagnostic, and codegen fills the missing argument by allocating the parameter type and loading it before the call: `%1 = alloca i32`, `%2 = load i32, ptr %1`, `call void @fbInit__FB_INIT(ptr %deref3, i32 %2)`. The member initialized from that parameter therefore gets an undefined value. The same holds for a return type on `FB_INIT`. Expected: a diagnostic at the declaration that `FB_INIT` takes no parameters and returns nothing.
```
// Reproducible example:
// plc --check: no diagnostic
...
P2 src/validation/statement.rs:776:13: Access modifiers are ignored, and every VAR member counts as private
`VAR PUBLIC`, `VAR PROTECTED`, and `VAR INTERNAL` behave exactly like a plain `VAR`: the E049 check tests `argument_type.is_private()`, which is `VariableType::Temp | VariableType::Local` (`src/index.rs:388`) and knows nothing about the modifier, so a read of a `PUBLIC` member from outside is reported as "Illegal access to private member". The same check compares the accessing POU with the container of the variable without following `EXTENDS`, so a `PROTECTED` member read inside a derived function block is reported as well. In the other direction, the modifiers on a `METHOD` are not checked at all: a `PRIVATE` method can be called from anywhere without a diagnostic. Expected: the modifier decides, `PROTECTED` includes the POUs that extend the declaring one, and a `PRIVATE` method is rejected outside its POU.
```
// Reproducible example:
// plc --check
// observed: E049 for b.open and for `guarded` inside Child; nothing for b.secret()
...
P2 src/parser.rs:591:17: ABSTRACT and FINAL on a POU are parsed and then ignored
The parser reads `ABSTRACT` and `FINAL` in front of a `FUNCTION_BLOCK` or `CLASS` into `PolymorphismMode`, and no other code in the compiler reads that field; `grep PolymorphismMode::` finds only the three writes in `parser.rs`. A `FUNCTION_BLOCK FINAL Leaf` can therefore be extended, and a `FUNCTION_BLOCK ABSTRACT Base` with an abstract method can be declared as a variable and called: the call returns `0`, because the abstract method has no body and the vtable slot holds the generated empty method. Both cases pass `--check` without a diagnostic. Expected: an error when a POU extends a `FINAL` POU, and an error when a variable takes an `ABSTRACT` type.
```
// Reproducible example:
...
FUNCTION_BLOCK Deeper EXTENDS Leaf   // expected: error, Leaf is FINAL
END_FUNCTION_BLOCK
...
P3 compiler/plc_driver/src/cli.rs:558:13: Help text for --header-prefix promises a default that is never applied
The help reads "The prefix for the generated header file(s). Will default to the project name if not supplied.", but an absent prefix becomes an empty string (`compiler/plc_driver/src/pipelines.rs:307`), and an empty prefix selects the naming after the source file instead. No run uses the project name. Expected: the help text describes the real default, or the default is implemented; note that applying the documented default would make every project with more than one source hit the overwrite described above.
```
// Reproducible example:
// plc generate plc.json headers   (project "demo", sources api.st and b.st)
// observed: api.h and b.h
// expected per the help text: demo.h
```

P3 src/parser/expressions_parser.rs:513:13: A cast binds to the name after the `#` although the parser's own comment says it binds to the whole chain
The comment above the cast branch reads "CAST-Statement: INT#a.b.c" and "this means INT#(a.b.c) rather than (INT#a).b.c", but the branch reads one atomic leaf after the `#`, and the enclosing loop then wraps every following step around the cast node, so `INT#a.b` parses as `(INT#a).b`. On a struct member that gives `error[E048]: Could not resolve reference to b`, because `INT#a` is an `INT` and an `INT` has no members; the message names the member and not the cast that caused it. Expected: the code and the comment agree, and a member step after a cast is reported in terms of the cast.
```
// Reproducible example:
...
P3 compiler/plc_driver/src/pipelines.rs:91:9: The variables that the build exports cannot be used in the project file that describes the build
`plc build` replaces every `$NAME` in `plc.json` with the value of the environment variable `NAME` (`resolve_environment_variables`, `compiler/plc_project/src/build_config.rs:119`), and the build exports `PROJECT_ROOT`, `BUILD_LOCATION`, and `LIB_LOCATION` for that purpose. The export happens at `pipelines.rs:96` and after, but `get_project` at `pipelines.rs:91` has already read and substituted the file, so none of the three is ever visible to it. A `"files": ["$PROJECT_ROOT/src/main.st"]` entry keeps the literal text and the build stops with `path '$PROJECT_ROOT/src/main.st' does not exist`. A variable that is already in the environment of the process (`SRCDIR=src plc build`) does resolve. `ARCH`, the placeholder for a per-architecture library path, is not exported anywhere in the compiler. Expected: the three locations are resolved before the project file is parsed.
```
// Reproducible example:
// plc build
// observed: error: path '$PROJECT_ROOT/src/hello.st' does not exist
// expected: the path resolves against the directory that holds plc.json
// --- file: plc.json ---
...
P3 compiler/plc_driver/src/pipelines.rs:318:5: The config subcommand accepts a format but always prints JSON
`plc config --format toml diagnostics` and `plc config --format toml schema` both print JSON. The `--format` option of the subcommand (`compiler/plc_driver/src/cli.rs:489`, values `json` and `toml`, default `json`) is parsed, carried in `ConfigFormat`, and handed to `print_config_options`, which prints `get_validation_schema()` and `get_diagnostic_configuration()` and never reads it. Expected: TOML output for `diagnostics`, or a rejection of the value.
```
// Reproducible example:
// plc config --format toml diagnostics
// observed: {"ignore":["E132","E015"],"warning":[...],...}
// expected: the same configuration as TOML
```
