...
Function and function blocks have similar properties, but they have fundamentally different representation in the compiler.
...
In contrast, a function block is backed by a struct and is globally accessible by a defined instance.
To declare a function block, you have to declare the backing struct and pass it as a reference to the function block implementation.
...
Note: Internally non primitive datatypes are always passed as pointers, they are however copied into a temporary variable before getting passed in order to preserve the by value properties. Using `{ref}` will circumvent this behaviour.
...
- A variable reprenting a Time should be stored in the appropriate time type and not an `LINT` or `LWORD`
...
Show examples of how the struct looks like + naming
Naming conventions
Mention init of FB

- make use of plc interface => dont just use a pointer to struct
- use adequate types and ptr to those types
