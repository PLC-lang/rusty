...
An diagnostics severity can be changed for example from `warning` to `error` or `info` and vice-versa or `ignore`d completely
To see a default error configuration use `plc config diagnostics`
To provide a custom error configuration use `plc --error-config <custom.json>`. This command is globally available and will work with other subcommands
