...
Participants rewrite the AST around the pipeline stages. The [Driver](../pipeline/00-driver.md#participants) explains their hooks. This part follows registration order; each chapter explains one transformation and its dependencies.

Hooks run in pipeline order. Within a hook, participants run in registration order. Thus a participant sees all rewrites from earlier hooks, plus those from earlier participants on the same hook. The hook implementations are in `compiler/plc_driver/src/pipelines/`; the Where column lists the transformation code.
...
| 3 | [Property](02-property.md) | `pre_index`, `post_annotate` | Accessor blocks into `__get_x` and `__set_x` methods, property uses into calls | Later stages use accessor methods and calls | `src/lowering/property.rs` |
...
| 11 | [Inheritance](10-inheritance.md) | `pre_index`, `post_annotate` | `EXTENDS` into an embedded member named after the base type, and accesses through it | Codegen has no notion of inheritance | `compiler/plc_lowering/src/inheritance.rs` |
...
A rewrite can require a new index, new annotations, or both. The Interactions sections explain these dependencies. After the transformations, the [Outputs](../outputs/README.md) chapters describe other uses of the resulting declarations and index.
