...
fn parse_prefix(input: &str) -> ParseResult<Prefix> {
    fn fn_prefix(input: &str) -> ParseResult<Prefix> {
        tag("fn").map(|_| Prefix::Func).parse(input)
    }
    fn var_prefix(input: &str) -> ParseResult<Prefix> {
        tag("var").map(|_| Prefix::Var).parse(input)
    }
