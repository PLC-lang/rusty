...
use crate::{
    SyntaxKind::{self, EOF, ERROR, TOMBSTONE}, TokenSet, T, event::Event, input::Input 
};
...
/// Entry point for parsing. Takes an Input and an entry point function, 
/// returns the event stream.
pub fn parse(inp: &Input, entry_point: fn(&mut Parser)) -> Vec<Event> {
...
impl<'t> Parser<'t> {
    ...
    pub(crate) fn eat(&mut self, kind: SyntaxKind) -> bool {
        ...
        //TODO: see if we actually need to distinguish these composite tokens in the lexer, or we can just lex them as separate tokens
        let n_raw_tokens = 1;
    ...
    pub(crate) fn bump_remap(&mut self, kind: SyntaxKind) {
        ...
        unimplemented!("bump_remap is not implemented yet, we need to decide how to handle the range for remapped tokens");
        // self.do_bump(kind, 1);
    }
    ...
    fn do_bump(&mut self, kind: SyntaxKind, n_raw_tokens: u8) {
        let range = self.inp.range(self.pos);
