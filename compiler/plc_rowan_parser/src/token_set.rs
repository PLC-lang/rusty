...
/// See #17037.
const LAST_TOKEN_KIND_DISCRIMINANT: usize = SyntaxKind::IDENT as usize; //TODO: what is the last token `SyntaxKind`? We should probably add a `LAST_TOKEN` variant to `SyntaxKind` and use that here.
...
fn token_set_works_for_tokens() {
    use crate::SyntaxKind::*;
    let ts = TokenSet::new(&[EOF, IDENT]);
    assert!(ts.contains(EOF));
    assert!(ts.contains(IDENT));
