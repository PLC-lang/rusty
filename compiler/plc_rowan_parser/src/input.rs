...
impl Input {
    ...
    #[inline]
    pub fn push(&mut self, kind: SyntaxKind, range: Range<u32>) {
        self.push_impl(kind, SyntaxKind::EOF, range)
    }
    #[inline]
    pub fn push_ident(&mut self, contextual_kind: SyntaxKind, range: Range<u32>) {
        self.push_impl(SyntaxKind::IDENT, contextual_kind, range)
    }
    ...
    #[inline]
    fn push_impl(&mut self, kind: SyntaxKind, contextual_kind: SyntaxKind, range: Range<u32>) {
        let idx = self.len();
...
impl Input {
    ...
    pub (crate) fn range(&self, idx: usize) -> Range<u32> {
        self.range.get(idx).cloned().unwrap_or(0..0)
    }
