...
impl<'lwr> AstLowerer<'lwr> {
    pub fn lower_init_functions(mut self, init_symbol_name: &str) -> Self {
        ...
        if let Some((mut new_index, init_unit)) =
            create_init_wrapper_function(&self, init_symbol_name)
        {
            self.index.import(std::mem::take(&mut new_index));
            let (a, deps, literals) = TypeAnnotator::visit_unit(&self.index, &init_unit, self.ctx.id_provider.clone());
...
fn create_init_wrapper_function(
    ...
    let mut globals = if let Some(stmts) = lowerer.unresolved_initializers.get("__global") {
        stmts
            .iter()
