...
impl AstNode {
    ...
    pub fn get_as_list(&self) -> Vec<&AstNode> {
        try_into_ref!(self, Vec<AstNode>).map(|it| it.iter().collect()).unwrap_or(vec![self])
    }
    ...
    pub fn get_label_name(&self) -> Option<&str> {
        try_into_ref!(self, LabelStatement).map(|it| it.name.as_str()).ok()
    }
    ...
    pub fn is_zero(&self) -> bool {
        try_into_ref!(self, AstLiteral).is_ok_and(|it| it.is_zero())
    }
    ...
    pub fn get_literal_integer_value(&self) -> Option<i128> {
        try_into_ref!(self, AstLiteral).map(|it| it.get_literal_integer_value()).unwrap_or_default()
    }
