...
pub fn compile<T: AsRef<str> + AsRef<OsStr> + Debug>(args: &[T]) -> Result<()> {
    ...
    if !compile_parameters.is_check() {
        ...
        if let Err(res) = res {
            ...
            return Err(Diagnostic::critical("Compilation aborted due to previous errors")
                .with_error_code(plc_diagnostics::errno::ErrNo::codegen__general)
                .into());
        }
    }
...
fn generate_to_string_internal<T: SourceContainer>(
    ...
    module.map(|it| it.persist_to_string()).ok_or_else(|| Diagnostic::critical("Cannot generate module"))
}
...
fn get_project(compile_parameters: &CompileParameters) -> Result<Project<PathBuf>> {
    ...
    let project = if let Some(command) = &compile_parameters.commands {
        ...
        let config = command
            ...
            .ok_or_else(|| {
                Diagnostic::error("Could not find 'plc.json'").with_error_code(ErrNo::general__param_err)
            })?;
