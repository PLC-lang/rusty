...
fn debug_compile_unit_uses_relative_path_when_source_is_outside_compile_dir() {
    ...
    let source = source_root.path().join("nested/main.st");
    write_test_source(&source);
    let source = source.canonicalize().expect("canonical source path");

    let compile_root = compile_root.path().canonicalize().expect("canonical compile root");
    ...
    let source_root_name = source_root.path().file_name().unwrap().to_string_lossy().to_string();
    ...
    let snapshot = sanitize_debug_snapshot(
        ...
        &[
            ...
            (&normalize_snapshot_paths(&source_root.path().to_string_lossy()), "/outside-root"),
