// Driver-level tests for `--file-prefix-map`, `--debug-prefix-map`, and
// `--debug-compilation-dir`.
//
// # Mirroring policy
//
// Tests live in two sibling modules: `linux` (compiled when
// `cfg(not(windows))`) and `windows` (compiled when `cfg(windows)`). The
// intent is that every scenario covered on one platform also has a mirror
// under the same test name on the other side — Windows path semantics (drive
// prefixes, `\\?\` verbatim prefixes, separator handling) diverge enough that
// regressions usually surface there first, so platform parity is what keeps
// the suite honest.
//
// When you add a test to `linux`, add a test with the same name to `windows`
// (and vice versa). If a scenario genuinely only makes sense on one OS — for
// example the drive-prefix guard `paths_use_virtual_roots_without_drive_prefixes`
// — that's fine; document it with a short comment so reviewers know it's
// intentionally platform-specific.
//
// Snapshot output may legitimately differ between platforms (Windows codegen
// splits the default-cwd case into two `!DIFile` records, Linux collapses
// them). Always run snapshots through `sanitize_debug_snapshot` so the text
// itself stays portable; let the inline literal carry the platform-specific
// shape.
...
mod linux {
    use super::{
        canonical_cwd, canonical_path, case_dir_name, normalize_snapshot_paths, relative_path_from,
        sanitize_debug_snapshot, write_test_source, Builder, Path, PathBuf,
    };
    use crate::{
    ...
    #[test]
    fn compile_unit_uses_path_relative_to_current_working_directory_by_default() {
...
"#;
    ...
    #[test]
    fn compile_unit_uses_relative_path_when_source_is_outside_compile_dir() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let compile_root =
            Builder::new().prefix("debug-paths-build-root-").tempdir_in(&cwd).expect("tempdir");
        let source_root = Builder::new()
            .prefix("debug-paths-source-root-")
            .tempdir_in(tempdir_base(&cwd))
            .expect("tempdir");
    ...
    #[test]
    fn project_config_inside_root_uses_relative_compile_unit_name() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let project_root =
            Builder::new().prefix("debug-paths-project-root-").tempdir_in(&cwd).expect("tempdir");
...
"#,
    ...
    #[test]
    fn project_config_outside_root_uses_relative_compile_unit_name() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let config_root =
            Builder::new().prefix("debug-paths-config-root-").tempdir_in(&cwd).expect("tempdir");
        let source_root = Builder::new()
            .prefix("debug-paths-source-root-")
            .tempdir_in(tempdir_base(&cwd))
            .expect("tempdir");
...
"#,
    ...
    #[test]
    fn constructor_generation_keeps_debug_paths_stable() {
...
"#;
    ...
    #[test]
    fn prefix_map_rewrites_debug_paths_without_leaking_local_paths() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
    ...
    #[test]
    fn mapped_source_root_outside_mapped_build_root_stays_absolute() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
    ...
    #[test]
    fn compilation_dir_mapping_keeps_parent_relative_filename_without_host_leakage() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir =
            Builder::new().prefix("debug-paths-parent-relative-").tempdir_in(&cwd).expect("tempdir");
    ...
    #[test]
    fn prefix_map_alias_matches_file_prefix_map() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir = Builder::new().prefix("debug-paths-alias-").tempdir_in(&cwd).expect("tempdir");
        let source = case_dir.path().join("src/main.st");
        write_test_source(&source, source_code);
        let source = canonical_path(&source);
...
mod windows {
    use super::{
        canonical_cwd, canonical_path, case_dir_name, normalize_snapshot_paths, relative_path_from,
        sanitize_debug_snapshot, write_test_source, Builder, PathBuf,
    };
    use crate::{
        tests::{compile_args_to_string, compile_build_config_to_string, compile_to_string_with_options},
        CompileOptions,
    ...
    // Windows-only: guards against drive-letter leakage in DWARF when virtual roots are in use.
    // No `linux` mirror — the failure mode (`C:\…` paths sneaking past the prefix map) does not
    // exist on Linux.
    #[test]
    fn paths_use_virtual_roots_without_drive_prefixes() {
        let source_code = r#"PROGRAM prg
VAR
    x : INT;
END_VAR
...
"#;
        ...
        // Captures Windows codegen behavior in the no-flags default: two `!DIFile` records are
        // emitted (source basename + dirname; compile-unit file with the full path). The Linux
        // mirror collapses both into a single record. The test exists to catch regressions, not
        // to bless the split — if the platforms ever converge the snapshot is the place to update.
        assert_snapshot!(snapshot, @r#"
        !2 = !DIFile(filename: "main.st", directory: "<CWD>/<CASE_ROOT>/src")
        !9 = distinct !DICompileUnit(language: DW_LANG_C, file: !10, producer: "RuSTy Structured text Compiler", isOptimized: false, runtimeVersion: 0, emissionKind: FullDebug, globals: !11, splitDebugInlining: false)
        !10 = !DIFile(filename: "<CWD>/<CASE_ROOT>/src/main.st", directory: "<CWD>")
        "#);
...
"#;
        ...
        let cwd = canonical_cwd();
        let project_root =
            Builder::new().prefix("debug-paths-win-project-root-").tempdir_in(&cwd).expect("tempdir");
...
"#;
        ...
        let cwd = canonical_cwd();
        let config_root =
            Builder::new().prefix("debug-paths-win-config-root-").tempdir_in(&cwd).expect("tempdir");
        let source_root =
            Builder::new().prefix("debug-paths-win-source-root-").tempdir_in(&cwd).expect("tempdir");
        ...
        let source = canonical_path(&source);
        let relative_outside = relative_path_from(config_root.path(), &source);
        // JSON requires forward slashes (or escaped backslashes) — Windows paths default to
        // backslashes which would produce invalid JSON.
        let relative_outside_json = relative_outside.to_string_lossy().replace('\\', "/");
...
"#,
    ...
    // The Linux mirror produces a single `!DIFile`. Windows codegen splits the no-flags default
    // into two records (basename + dir, plus a compile-unit file with the full path) — see
    // `compile_unit_uses_path_relative_to_current_working_directory_by_default` above for the
    // same divergence. The constructor flags do not change that shape.
    #[test]
    fn constructor_generation_keeps_debug_paths_stable() {
        let source_code = r#"FUNCTION_BLOCK foo
VAR
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir = Builder::new().prefix("debug-paths-win-ctors-").tempdir_in(&cwd).expect("tempdir");
        let source = case_dir.path().join("test.st");
        write_test_source(&source, source_code);
        let source = canonical_path(&source);
        ...
        assert_snapshot!(snapshot, @r#"
        !2 = !DIFile(filename: "test.st", directory: "<CWD>/<CASE_ROOT>")
        !10 = !DIFile(filename: "<CWD>/<CASE_ROOT>/test.st", directory: "<CWD>")
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir =
            Builder::new().prefix("debug-paths-win-prefix-map-").tempdir_in(&cwd).expect("tempdir");
        let source = case_dir.path().join("src/main.st");
        write_test_source(&source, source_code);
        let source = canonical_path(&source);

        // Use a literal virtual root rather than the Linux helper's tempdir-parent path. The
        // value is a string-only marker; it never has to exist on disk.
        let virtual_root = PathBuf::from("/src/TestProject");
        let ir = compile_to_string_with_options(
            vec![source],
            vec![],
            CompileOptions {
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir =
            Builder::new().prefix("debug-paths-win-virtual-roots-").tempdir_in(&cwd).expect("tempdir");
        let source = case_dir.path().join("src/main.st");
        write_test_source(&source, source_code);
        let source = canonical_path(&source);
...
"#;
        ...
        let cwd = canonical_cwd();
        let case_dir =
            Builder::new().prefix("debug-paths-win-parent-relative-").tempdir_in(&cwd).expect("tempdir");
        let compile_dir = case_dir.path().join("test");
        fs::create_dir_all(&compile_dir).expect("failed to create compile dir");
