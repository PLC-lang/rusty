...
## [2.0.0](https://github.com/PLC-lang/rusty/releases/tag/v2.0.0) - 2026-09-24

### Added

- **cfc**: Emit diagram locations in debug info (#1910)
- **cfc**: Report diagram locations in diagnostics (#1909)
- **parser**: Accept CONSTANT on VAR_INPUT as a no-op (#1885)
- **cfc**: Storage mode (REF= assignment) (#1891)
- **cfc**: Execution control (EN/ENO) (#1880)
...
- Ensuring that TO_STRING conversion functions do not panic (#1903)
- **codegen**: Store struct literal members with assignment semantics (#1912)
- **codegen**: Anchor debug locations to their own statement (#1900)
- **stdlib**: Parse STRING_TO_* input with the compiler's literal grammar (#1907)
- **codegen**: Null-terminate untyped variadic argument lists (#1887)
- **cfc**: Adapt to new return pin format (#1906)
- **validation**: Remove VAR_INPUT {ref} warning (E042) (#1892)
...
- **debug**: Add configurable DWARF path remapping for shipped builds (#1692)
- **cli**: Make build-location global and clarify artifact paths (#1697)
- Add AND_THEN/OR_ELSE and make AND/OR eager (#1704)
- Migrate property syntax to IEC 61131-3 2025 edition (#1688)

### Fixed

- Avoid duplicate inherited FB initialization (#1702)
- **resolver**: Resolve ReferenceExpr base under default scopes (#1707)
- **validation**: Report typedef of unknown type at the typedef site (#1711)
- **validation**: Reject incomplete hardware addresses in FUNCTION/METHOD (#1712)
- **pre-processor**: Apply inline-type pre-processing to VAR_CONFIG (#1715)
- **index**: Dedup synthetic hardware-backing globals across units (#1716)
- **validation**: Detect case-insensitive POU name collisions with generics (#1705)
- **codegen,parser**: Correctly deref RValue pointers; allow ^ after a call (#1714)
- Reclassify formal and informal mixed args diagnostic (#1703)
- Improve diagnostics for missing property accessors (#1700)
