...
mod tests {
    ...
    fn nanos(year: i32, month: u32, day: u32, hour: u32, min: u32, sec: u32, nano: u32) -> i64 {
    ...
    fn formats_requested_values() {
        ...
        unsafe { TIME_TO_STRING(dest.as_mut_ptr(), -1) };
        assert_eq!("T#213503d23h34m33s709ms551us615ns", terminated_str(&dest));
        unsafe { LTIME_TO_STRING(dest.as_mut_ptr(), -1) };
        assert_eq!("LTIME#213503d23h34m33s709ms551us615ns", terminated_str(&dest));
        unsafe { DATE_TO_STRING(dest.as_mut_ptr(), nanos(2106, 2, 7, 6, 28, 15, 0)) };
        assert_eq!("D#2106-02-07", terminated_str(&dest));
        unsafe { DT_TO_STRING(dest.as_mut_ptr(), nanos(2106, 2, 7, 6, 28, 15, 0)) };
        assert_eq!("DT#2106-02-07-06:28:15", terminated_str(&dest));
        unsafe { TOD_TO_STRING(dest.as_mut_ptr(), 86_399_999 * NANOS_PER_MILLISECOND as i64) };
        assert_eq!("TOD#23:59:59.999", terminated_str(&dest));
    }

    #[test]
    fn short_and_long_variants_keep_nanosecond_precision() {
        let mut dest = [0_u8; STRING_CAPACITY];
        let timestamp = nanos(1982, 12, 15, 10, 10, 2, 123_456_789);

        unsafe { TIME_TO_STRING(dest.as_mut_ptr(), 1_000_400) };
        assert_eq!("T#1ms400ns", terminated_str(&dest));
        unsafe { DT_TO_STRING(dest.as_mut_ptr(), timestamp) };
        assert_eq!("DT#1982-12-15-10:10:02.123456789", terminated_str(&dest));
        unsafe { TOD_TO_STRING(dest.as_mut_ptr(), 86_399_999_999_999) };
        assert_eq!("TOD#23:59:59.999999999", terminated_str(&dest));
        unsafe { DATE_TO_STRING(dest.as_mut_ptr(), -1) };
        assert_eq!("D#1969-12-31", terminated_str(&dest));
    }
    ...
    fn date_to_string_is_converted_in_correct_format() {
        ...
        unsafe { LDATE_TO_STRING(dest.as_mut_ptr(), nanos(1982, 12, 15, 10, 10, 2, 123_456_789)) };
    ...
    fn dt_to_string_is_converted_in_correct_format() {
        ...
        unsafe { LDT_TO_STRING(dest.as_mut_ptr(), nanos(1982, 12, 15, 10, 10, 2, 123_456_789)) };
    ...
    fn tod_to_string_is_converted_in_correct_format() {
        ...
        unsafe { LTOD_TO_STRING(dest.as_mut_ptr(), nanos(1982, 12, 15, 10, 10, 2, 123_456_789)) };
    ...
    fn time_to_string_is_converted_in_correct_format() {
        ...
        unsafe { LTIME_TO_STRING(dest.as_mut_ptr(), nanos(1982, 12, 15, 10, 10, 2, 123_456_789)) };
        assert_eq!("LTIME#4731d10h10m2s123ms456us789ns", terminated_str(&dest));
    }

    #[test]
    fn temporal_extremes_do_not_panic() {
        let mut dest = [0_u8; STRING_CAPACITY];

        for input in [i64::MIN, -1, 0, i64::MAX] {
            unsafe { TIME_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { LTIME_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { DT_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { LDT_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { DATE_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { LDATE_TO_STRING(dest.as_mut_ptr(), input) };
            unsafe { TOD_TO_STRING(dest.as_mut_ptr(), input) };
