    ...
    use plc::{
        ...
        DebugLevel, OnlineChange, validation::Validator,
    };
    ...
    pub fn annotate_and_lower_with_ids(
        ...
        let full_index = aggregate_lowerer.index.take().unwrap();
        let mut inheritance_lowerer = InheritanceLowerer::new(id_provider.clone());
        inheritance_lowerer.index.replace(full_index);
        inheritance_lowerer.annotations.replace(Box::new(all_annotations));
        units.iter_mut().for_each(|unit| {
            inheritance_lowerer.visit_unit(unit);
        });
        let mut full_index = inheritance_lowerer.index.take().unwrap();
        let mut all_annotations = AnnotationMapImpl::default();
    ...
    pub fn codegen_with_online_change(src: &str) -> String {
        codegen_debug_without_unwrap_oc(
            src,
            DebugLevel::None,
            OnlineChange::Enabled { file_name: "test".into(), format: plc::ConfigFormat::JSON },
        )
        .unwrap()
    }
    ...
    pub fn codegen_with_debug(src: &str) -> String {
        codegen_debug_without_unwrap(src, DebugLevel::Full(plc::DEFAULT_DWARF_VERSION)).unwrap()
    }
