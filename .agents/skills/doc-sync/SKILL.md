...
The technical book in `book/technical/` describes how the compiler works. Code and book are two views of the same thing, and a change to one view is complete only when the other still agrees with it. This skill checks that agreement for the changes in the current branch.
