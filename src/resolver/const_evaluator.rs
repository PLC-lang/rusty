...
fn evaluate_with_target_hint(
    ...
    let literal = match initial {
        ...
        AstStatement::ReferenceExpr { access: ReferenceAccess::Cast(target), base:Some(type_name),.. } => {
            let dti = type_name.get_flat_reference_name().and_then(|type_name| index.find_effective_type_info(type_name));
            match dti {
...
use plc_ast::{
    ast::{AstId, AstStatement, Operator, ReferenceAccess, SourceRange, TypeNature},
