...
fn function_block_init_fn_created() {
    ...
    // Find the foo_ctor implementation and verify its properties
    ...
    // Find the foo_ctor POU and verify it has a "self" parameter
    let foo_ctor_pou = unit.pous.iter().find(|pou| pou.name == "foo_ctor").expect("foo_ctor POU not found");
...
fn init_wrapper_function_created() {
    ...
    // we expect to find a `__unit___internal___ctor` function in the compilation unit
    ...
    let implementation = unit
        ...
        .expect("__unit___internal___ctor implementation not found in unit");
    ...
    assert!(unit.pous.iter().any(|pou| pou.name == "bar_ctor"));
    assert!(unit.pous.iter().any(|pou| pou.name == "foo_ctor"));
}
