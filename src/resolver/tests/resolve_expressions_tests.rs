...
#[test]
fn array_of_struct_as_member_of_another_struct() {
    let id_provider = IdProvider::default();
    let (unit, mut index) = index_with_ids(
        "
		PROGRAM mainProg
        VAR
            var_str1 : STRUCT1;
        END_VAR
            var_str1 := ((name := 'text'), (node := 1), (myArr := (x1 := FALSE, x2 := TRUE, x3 := 23, x4 := 100)));
        END_PROGRAM
    ...
    let AstStatement::Assignment { left, right, .. } = &unit.implementations[0].statements[0] else {
        panic!("expected assignment")
    };

    assert_type_and_hint!(&annotations, &index, left.as_ref(), "STRUCT1", None);

    let AstStatement::Assignment { left, right, .. } = ast::flatten_expression_list(right.as_ref())[2] else {
        panic!("expected assignment")
    };

    assert_type_and_hint!(&annotations, &index, left.as_ref(), "__STRUCT1_myArr", None);
    assert_type_and_hint!(&annotations, &index, right.as_ref(), "", Some("__STRUCT1_myArr"));

    let assigments = ast::flatten_expression_list(right.as_ref());
    let assignments_destructured = assigments
        .iter()
        .map(|it| {
            let AstStatement::Assignment { left, right, .. } = it else {
                panic!("expected assignment")
            };
    ...
    let types = ["BOOL", "BOOL", "DINT", "DINT"];

    for (idx, (left, right)) in assignments_destructured.iter().enumerate() {
        let annotation = format!("__STRUCT2_x{idx}");
        assert_type_and_hint!(&annotations, &index, *left, annotation.as_str(), None);
        assert_type_and_hint!(&annotations, &index, *right, types[idx], Some(annotation.as_str()));
    }
}
