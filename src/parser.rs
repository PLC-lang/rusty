...
use plc_ast::{
    ast::{
        AccessModifier, ArgumentProperty, AstFactory, AstNode, AstStatement, AutoDerefType, CompilationUnit, ConfigVariable, DataType, DataTypeDeclaration, DirectAccessType, GenericBinding, HardwareAccessType, Implementation, LinkageType, PolymorphismMode, Pou, PouType, ReferenceAccess, ReferenceExpr, TypeNature, UserTypeDeclaration, Variable, VariableBlock, VariableBlockType
    },
    provider::IdProvider,
};
use plc_diagnostics::{
...
fn parse_variable_list(lexer: &mut ParseSession) -> Vec<Variable> {
    ...
    while lexer.token == Identifier { // double while token == ident?
...
fn parse_config_variables(lexer: &mut ParseSession) -> Vec<ConfigVariable> {
    ...
    while lexer.token == Identifier {
        let Some(configured_var) = try_parse_config_var(lexer) else { continue; };
...
fn try_parse_config_var(lexer: &mut ParseSession) -> Option<ConfigVariable> {
    ...
    parse_data_type_definition(lexer, None).map(|(dt, init)| {
        if init.is_some() {
            lexer.accept_diagnostic(Diagnostic::unexpected_token_found(format!("{KeywordSemicolon:?}").as_str(), "Initializer", lexer.last_location().span(&lexer.location())))
        }
        lexer.try_consume(&KeywordSemicolon);
