...
pub fn generate_data_types<'ink>(
    ...
    for (name, user_type) in types.iter() {
        if user_type.get_type_information().is_interface() {
            // At codegen, interfaces types no longer exist and instead are replaced by fat pointers
            // TODO: Link to module responsible for lowering interfaces to fat pointers
            continue;
        }
