...
use crate::test_utils::tests::{codegen, codegen_with_profile, generate_with_empty_program};
...
fn program_with_and_then_statement() {
    // AND_THEN always short-circuits (uses branches + phi), regardless of profile
    let result = codegen(
        r#"PROGRAM prg
VAR
x : BOOL;
...
fn program_with_or_else_statement() {
    // OR_ELSE always short-circuits (uses branches + phi), regardless of profile
    let result = codegen(
        r#"PROGRAM prg
VAR
x : BOOL;
...
#[test]
fn program_with_and_statement_standard_profile() {
    // Under the standard profile, AND short-circuits (same as AND_THEN)
    let result = codegen_with_profile(
        r#"PROGRAM prg
VAR
x : BOOL;
y : BOOL;
...
END_PROGRAM
"#,
        plc_diagnostics::profiles::CompatibilityProfile::standard(),
    );
    filtered_assert_snapshot!(result, @r#"
    ; ModuleID = '<internal>'
    source_filename = "<internal>"
...
#[test]
fn program_with_or_statement_standard_profile() {
    // Under the standard profile, OR short-circuits (same as OR_ELSE)
    let result = codegen_with_profile(
        r#"PROGRAM prg
VAR
x : BOOL;
