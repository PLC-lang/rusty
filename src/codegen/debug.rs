...
use inkwell::{
    ...
    types::BasicTypeEnum,
    values::{
        BasicMetadataValueEnum, CallSiteValue, FunctionValue, GlobalValue, InstructionOpcode,
...
use plc_llvm::{DebugInfoBuilderExt, InstructionValueExt};
...
pub trait Debug<'ink> {
    ...
    /// of the debug builder has been finalized. It also marks the key instruction of every
    /// statement, so every function body must be generated by then.
    fn finalize(&self);
}
...
pub struct DebugBuilder<'ink> {
    ...
    /// The functions whose debugger stops are placed by key instructions, marked at `finalize`.
    key_functions: Vec<FunctionValue<'ink>>,
...
impl<'ink> Debug<'ink> for DebugBuilder<'ink> {
    ...
    fn register_function<'idx>(
        ...
        self.debug_info.enable_key_instructions(func.function);
        self.key_functions.push(func.function);
    ...
    fn finalize(&self) {
        // One key per statement, in an atom group unique within its function.
        for function in &self.key_functions {
            let mut group = 0;
            for block in function.get_basic_blocks() {
                for key in statement_keys(block) {
                    group += 1;
                    key.set_key_instruction(group, 1);
                }
            }
        }
...
// The key instruction of each statement in the block. A statement is a run of instructions at one
// line; its key is its last side effect, or its last instruction if it has none.
fn statement_keys(block: BasicBlock) -> Vec<InstructionValue> {
    ...
    for instruction in block.get_instructions().filter(|it| !is_lifetime_marker(*it)) {
        ...
        let line = Some((location.get_line(), location.get_scope()));
        if line != statement {
            keys.extend(key.take());
            statement = line;
        }

        // A side effect always takes the key; another instruction only while no side effect has.
        if has_side_effect(instruction) || !key.is_some_and(has_side_effect) {
...
fn has_side_effect(instruction: InstructionValue) -> bool {
    matches!(
...
// Lifetime markers emit no machine code, so a debugger could never stop at one.
fn is_lifetime_marker(instruction: InstructionValue) -> bool {
    CallSiteValue::try_from(instruction)
        .ok()
        .and_then(|call| call.get_called_fn_value())
        .is_some_and(|callee| callee.get_name().to_bytes().starts_with(b"llvm.lifetime"))
}
