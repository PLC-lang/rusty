...
impl<'a, 'b> StatementCodeGenerator<'a, 'b> {
    pub fn new(
        ...
        linking_context: Option<&'b FunctionContext<'a>>,
    ...
    fn generate_assignment_statement(
        ...
        let left = self.generate_l_value(left_statement)?;
        let (right_type, right) = {
            ...
            sub_statement_gen.generate_expression(right_statement)?
        };
        let cast_value =
        ...
        let instance_and_index_entry = match operator {
            Statement::Reference { name, .. } => {
                ...
                let callable_reference = if let Some(variable_instance) = variable {
                    ...
                    let instance_generator = InstanceStructGenerator::new(self.llvm, self.index);
                    let callable = instance_generator.allocate_struct_instance(&name, &operator.get_location())?;
                    callable
                };
    ...
    fn generate_for_statement(
        ...
        let (_, counter_statement) = self.generate_expression(counter)?;
        let (_, end_statement) = self.generate_expression(end)?;
        ...
        let (_, step_by_value) = by_step
             ...
             |step| self.generate_expression(&step))?;
             

        let next = builder
            .build_int_add(counter_statement.into_int_value(), step_by_value.into_int_value(), "tmpVar");
                    
        let ptr = self.generate_lvalue_for(counter)?.ptr_value;
    ...
    fn generate_case_statement(
        ...
        let (_, selector_statement) = self.generate_expression(&*selector)?;
        ...
        for i in 0..conditional_blocks.len() {
            ...
            let (_, condition) = self.generate_expression(&*conditional_block.condition)?; //TODO : Is a type conversion needed here?
    ...
    fn generate_base_while_statement(
        ...
        //Check loop condition
        builder.position_at_end(condition_check);
        let (_, condition_value) = self.generate_expression(condition)?;
        builder
    ...
    fn generate_if_statement(
        ...
        for (i, block) in conditional_blocks.iter().enumerate() {
            ...
            let (_,condition) = self.generate_expression(&block.condition)?;
    ...
    pub fn generate_expression(
        ...
        match statement {
            Statement::Reference { name, location } => {
                let pointer =
                    self.create_llvm_pointer_value_for_reference(None, name, location)?;
    ...
    fn generate_phi_expression(
        ...
        let builder = &self.llvm.builder;
        let current_function = self.function_context.map(|it| it.function).unwrap();
        let right_branch = self.llvm.context.append_basic_block(current_function, "");
        let continue_branch = self.llvm.context.append_basic_block(current_function, "");
    ...
    pub fn generate_literal(&self, statement: &Statement) -> Result<TypeAndValue<'a>, CompileError> {
        match statement {
            ...
            Statement::LiteralReal { value, .. } => {
                self.llvm.create_const_real(self.index, &self.type_hint, value)
            },
            Statement::LiteralString { value, .. } => {
                self.llvm.create_const_string(value.as_str())
            },
