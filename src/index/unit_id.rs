...
impl UnitSymbolIndex {
    ...
    /// Returns true if any unit other than `exclude` claims ownership of a
    /// symbol with the same `kind`/`map_key`/`identifier`. Used by
    /// `Index::remove_unit` to keep entries that are co-owned by other units
    /// (auto-generated POUs and implementations whose names collide across
    /// compilation units): only the *last* owner being dropped should
    /// actually remove the entry from the index.
    pub fn has_owner_other_than(
        &self,
        exclude: UnitId,
        kind: SymbolKind,
        map_key: &str,
        ...
        self.entries.iter().any(|(uid, syms)| {
            *uid != exclude
                && syms.iter().any(|s| s.kind == kind && s.map_key == map_key && s.identifier == identifier)
        })
    }
}
