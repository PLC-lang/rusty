...
pub mod tests {
    ...
    // TODO: Remove this, only a temporary solution
    pub fn codegen_with_debug_cfc(src: &str) -> String {
        codegen_debug_without_unwrap_cfc(src, DebugLevel::Full).unwrap()
    }

    // TODO: Remove this, only a temporary solution
    pub fn codegen_debug_without_unwrap_cfc(src: &str, debug_level: DebugLevel) -> Result<String, String> {
        fn do_index<T: Into<SourceCode>>(
            src: T,
            id_provider: IdProvider,
        ) -> (CompilationUnit, Index, Vec<Diagnostic>) {
