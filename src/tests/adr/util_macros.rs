...
macro_rules! deconstruct_assignment {
    ($src:expr) => {{
        if let plc_ast::ast::AstNode { stmt: plc_ast::ast::AstStatement::Assignment(data), .. } = $src {
...
macro_rules! deconstruct_call_statement {
    ($src:expr) => {{
        if let plc_ast::ast::AstNode { stmt: plc_ast::ast::AstStatement::CallStatement(data), .. } = $src {
            (
                &data.operator,
...
macro_rules! deconstruct_binary_expression {
    ($src:expr) => {{
        if let plc_ast::ast::AstNode { stmt: plc_ast::ast::AstStatement::BinaryExpression(data), .. } = &$src
        {
            (&data.left, &data.right)
        } else {
            unreachable!();
