...
use crate::{
    codegen::generators::{expression_generator::{self, ExpressionCodeGenerator, ExpressionValue}, expression_visitor::{ExpressionVisitor, GeneratedValue} },
    index::Index,
    lexer, parser,
    resolver::{
        self,
...
lazy_static! {
    static ref BUILTIN: FxHashMap<&'static str, BuiltIn> = FxHashMap::from_iter([
            ...
            ",
                ...
                code: |generator, params, location| {
                    if let [reference] = params {
                        generator.generate_expression(reference)?
                            .into_r_value()
                            .map_err(|e| Diagnostic::codegen_error(
                                "{e}",
                                &location,
                            ))
                    } else {
        ...
        //             let function_context = generator.get_function_context()
        //                 .map_err(|e| Diagnostic::codegen_error("{e}", &location))?;
...
type CodegenFunction = for<'ink, 'b> fn(
    &mut ExpressionVisitor<'ink, 'b>,
    &[&AstNode],
    SourceLocation,
) -> Result<GeneratedValue<'ink>, Diagnostic>;
...
impl BuiltIn {
    pub fn codegen<'ink, 'b>(
        &self,
        generator: &mut ExpressionVisitor<'ink, 'b>,
        params: &[&AstNode],
        location: SourceLocation,
    ) -> Result<GeneratedValue<'ink>, Diagnostic> {
