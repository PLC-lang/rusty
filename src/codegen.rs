...
use std::{
    cell::RefCell, collections::HashMap, fmt::Display, ops::Deref, path::{Path, PathBuf}, sync::Mutex
};
...
use inkwell::{
    builder::BuilderError, context::Context, execution_engine::{ExecutionEngine, JitFunction}, memory_buffer::MemoryBuffer, support::LLVMString, types::BasicType, values::BasicValue, AddressSpace
};
use inkwell::{
    module::Module,
...
impl<'ink> CodeGen<'ink> {
    ...
    pub fn generate(
        ...
        #[cfg(feature = "verify")]
        {
            self.module
                .verify()
                .map_err(|it| CodegenError::from(it))
...
impl<'ink> GeneratedModule<'ink> {
    ...
    fn persist_to_obj(
        ...
        let machine = target
            ...
            .ok_or_else(|| {
                CodegenError::new("Cannot create target machine.", SourceLocation::undefined())
            });
...
impl From<CodegenError> for Diagnostic {
    fn from(err: CodegenError) -> Self {
        ...
        Diagnostic::new(format!("Builder error: {}", err.to_string()))
