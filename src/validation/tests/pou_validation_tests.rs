...
#[test]
fn property_inheritance_in_fb_with_conflicting_signature_on_same_accessors() {
    let source = r"
    FUNCTION_BLOCK fb1
        PROPERTY prop : DINT
            GET END_GET
...
#[test]
#[ignore = "PRG-2511"]
fn property_inheritance_in_fb_with_conflicting_signature_on_different_accessors() {
    let source = r"
    FUNCTION_BLOCK fb1
        PROPERTY prop : DINT
            GET END_GET
    ...
    assert!(!parse_and_validate_buffered(source).is_empty(), "missing validation");
}
