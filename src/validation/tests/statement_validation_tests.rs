...
fn assignment_of_incompatible_types_is_reported() {
    ...
    let ranges = &[(243..259), (290..307), (337..353), (384..401)];
    let types = &[("DINT", "STRING"), ("__prog_array_", "STRING"), ("STRING","DINT"), ("STRING", "__prog_array_")];
    for (idx, diag) in diagnostics.iter().enumerate() {
        assert_eq!(diag, &Diagnostic::invalid_assignment(types[idx].0, types[idx].1, ranges[idx].to_owned().into()))
    }
}
