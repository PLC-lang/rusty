...
fn builtin_functions_named_arguments_invalid_parameter_names() {
    ...
    error[E037]: Invalid assignment: cannot assign '// MOVE with wrong parameter name arr2 := MOVE(SOURCE := arr); // SIZEOF with wrong parameter name' to 'ARRAY[0..5] OF INT'
       ┌─ <internal>:14:13
       │
    14 │             arr2 := MOVE(SOURCE := arr);
       │             ^^^^^^^^^^^^^^^^^^^^^^^^^^^ Invalid assignment: cannot assign '// MOVE with wrong parameter name arr2 := MOVE(SOURCE := arr); // SIZEOF with wrong parameter name' to 'ARRAY[0..5] OF INT'
