...
fn pointer_to_validates_types_when_assigned_with_ref_call_in_initializer() {
    ...
    FUNCTION main
        VAR
            ...
            // Invalid assignments
            ptr         : POINTER TO DINT := REF(value_real);
            ptr         : POINTER TO DINT := REF(value_wstr);
            ptr         : POINTER TO DINT := REF(value_pos2d);
            ptr         : POINTER TO DINT := REF(value_pos3d);
