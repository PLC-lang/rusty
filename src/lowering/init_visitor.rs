use super::initializers::{get_user_init_fn_name, Init, InitAssignments, Initializers, GLOBAL_SCOPE};
use crate::{
    index::{get_init_fn_name, Index, PouIndexEntry, VariableIndexEntry},
    lowering::helper::{
        create_assignment, create_call_statement, create_member_reference, create_ref_assignment, Context,
    },
...
pub struct InitVisitor {
    pub index: Index,
    pub unresolved_initializers: Initializers,
    pub var_config_initializers: Vec<AstNode>,
    pub user_inits: FxHashMap<String, bool>,
    pub ctxt: Context,
}
...
impl InitVisitor {
    ...
    fn collect_user_init_candidates(&mut self, unit: &mut CompilationUnit) {
        ...
        for pou in unit
            .pous
            .iter()
            .filter(|it| matches!(it.kind, PouType::FunctionBlock | PouType::Program))
            .filter(|it| !matches!(it.linkage, LinkageType::BuiltIn))
        {
        ...
        for user_type in unit
            .user_types
            .iter_mut()
            .filter(|it| matches!(it.data_type, DataType::StructType { .. }))
            .filter(|it| !matches!(it.linkage, LinkageType::BuiltIn))
        {
