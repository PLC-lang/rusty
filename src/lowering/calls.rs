...
impl AstVisitorMut for AggregateTypeLowerer {
    ...
    fn visit_call_statement(&mut self, node: &mut AstNode) {
        ...
        //Get the function being called. Generic calls have already been rewritten to their concrete
        //monomorphization by the `GenericLowerer` phase, so the operator here always names a concrete
        //function (or function pointer).
        let (qualified_name, return_type_name) =
