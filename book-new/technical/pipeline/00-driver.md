...
The driver is the entry point of the compiler. It resolves the command line into a project, runs the pipeline stages in order, and hands the generated objects to the linker. Every other chapter in this part describes one stage that the driver calls; this chapter describes the frame around them.
...
Codegen runs in one of two modes. By default each unit becomes its own LLVM module and object file, written under the build location with a path that mirrors the source path. With `--single-module`, and always with `-c` (one output object can only come from one module), all units are merged into one module first. Linking then branches on the output format: IR and bitcode files are merged without a linker, a single object is copied, and everything else goes through the linker.

A project comes either from a build description (`plc.json`) or from the positional arguments of a direct `plc` call in the terminal. Its inputs fall into four groups:
...
- **Mutating participants** take the project by value and return a new one. They are the lowerers described above and use the four hooks around index and annotate. Diagnostics they collect while rewriting are gathered after the last `post_annotate` hook and reported together with the validation diagnostics.
- **Read-only participants** receive a shared reference and cannot change the project. They see all six hooks plus one call per generated module. The only default read-only participant is the codegen participant, which persists modules to disk and links them.
...
> **Developer Note**
>
> Participants started small. With only a few lowering steps, a dedicated IR would have been a large architectural change for little immediate gain, so rewriting the AST in place and re-running index and annotate was the pragmatic choice. The mechanism then grew by accretion, one lowering step per language feature, without a point at which the design was reconsidered. Today it works, but it carries known technical debt:
>
